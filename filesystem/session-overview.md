...
The `filesystem` server (`npx -y @modelcontextprotocol/server-filesystem /private/tmp`)
exposes **14 tools** via stdio transport, requiring no authentication. The server
restricts all operations to the `/private/tmp` allowed directory tree.

## Tool selection

The non-interactive gate was applied as documented in the eval harness:

**Probed (10 tools, all non-mutating):**
- `list_allowed_directories`, `list_directory`, `list_directory_with_sizes`,
  `directory_tree`, `get_file_info`, `search_files`, `read_file`, `read_text_file`,
  `read_multiple_files`, `read_media_file`

**Skipped (4 tools, mutating by keyword or semantic):**
- `write_file` — keyword `write` in name; creates/overwrites files
- `create_directory` — keyword `create` in name; creates filesystem entries
- `edit_file` — semantically mutating (makes line-based edits to files); skipped conservatively even though `edit` is not in the keyword list
- `move_file` — semantically mutating (moves/renames files); skipped conservatively even though `move` is not in the keyword list

## Response characteristics

All probed tools returned **plain text strings** (`_observed_shape: "str"`), not JSON
objects. This is by design:

- `list_allowed_directories` returns human-readable text like `"Allowed directories:\n/private/tmp"`
- `list_directory` / `list_directory_with_sizes` return `[FILE]`/`[DIR]` prefixed text listings
- `directory_tree` returns an error string (`EACCES: permission denied`) when it
  encountered permission-denied subdirectories in `/private/tmp`
- `get_file_info` returns multi-line `key: value` text (size, timestamps, permissions)
- `read_text_file` / `read_file` return the raw file content as a string
- `read_media_file` failed when given a `.txt` file (MCP protocol error: content type
  mismatch); succeeded once probed with a real PNG file and returned a string
- `search_files` returns a newline-separated list of matching paths
- `read_multiple_files` succeeded; the probe tool had a cosmetic TypeError in its
  `probed_args` display (list element unhashable) but the probe itself wrote correctly

## Discriminator advisory analysis

The `mcp-kit list` advisory flagged `path`, `head`, and `tail` as discriminator
candidates shared across multiple tools. On inspection these are **false positives**:
- `path` is a path parameter — the response shape does not vary by path value type,
  only by what file/dir happens to be there
- `head` / `tail` are line-count filters — they don't change the schema, only truncate
- No real discriminator patterns were found; all tools return `str` regardless

## Mode assignment

Since every tool returns a plain text string, there is no envelope to unwrap and no
`TypedDict` model to name. All tools are mode **A** (return type stays `Any` / `str`).
No Path-F guards were triggered (no nested dict structures observed).

| Tool | Mode | Reason |
|---|---|---|
| `list_allowed_directories` | A | Returns plain text string |
| `list_directory` | A | Returns `[FILE]/[DIR]` formatted text |
| `list_directory_with_sizes` | A | Returns `[FILE]/[DIR]` formatted text with sizes |
| `directory_tree` | A | Returns string (JSON-like tree text or error string) |
| `get_file_info` | A | Returns `key: value` text block |
| `search_files` | A | Returns newline-separated path list as text |
| `read_file` | A | Returns raw file content as text |
| `read_text_file` | A | Returns raw file content as text |
| `read_multiple_files` | A | Returns concatenated file contents as text |
| `read_media_file` | A | Returns string (base64 data or error text) |
| `write_file` | — | Skipped: mutating (`write` keyword) |
| `create_directory` | — | Skipped: mutating (`create` keyword) |
| `edit_file` | — | Skipped: mutating (semantic) |
| `move_file` | — | Skipped: mutating (semantic) |

## Generated module

`filesystem/filesystem.py` (7835 bytes, 14 async functions) parsed cleanly with
`ast.parse`. All signatures match the server's `inputSchema`. Return types remain
`Any` across all tools (correct: the server returns text, not dicts). The shapes
sidecar carries honest `_observed_shape: "str"` evidence for all 10 probed tools.
