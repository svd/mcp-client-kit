...
# Session Overview — context7 eval

## Server summary

**Server:** context7 (`npx -y @upstash/context7-mcp`, stdio transport, no auth)
**Version reported:** Context7 Documentation MCP Server v3.2.1

The server exposes **2 tools**: `resolve-library-id` and `query-docs`. Both tools are
non-mutating (lookup/read-only) and were probed. No tools were skipped.

## Tool selection (Step 2)

```
Tools on context7:
  resolve-library-id — Resolves a package/product name to a Context7-compatible library ID
  query-docs         — Retrieves documentation and code examples for a library ID
```

No tools contain mutating keywords (`create`, `update`, `delete`, `send`, etc.), so
**both tools were selected for probing**. None were skipped.

The `mcp-kit list` discriminator advisory flagged `query` as a shared param spanning
both tools. However, `query` is a free-text user search string, not an enum that
drives different response schemas. Both tools return `str` regardless of query content,
so the advisory is a false positive for this server. No discriminator resolution was
needed.

## Probing (Step 3)

Both tools were probed with minimal safe values:

- `resolve-library-id`: `{"query": "react"}` — a common library name
- `query-docs`: `{"context7CompatibleLibraryID": "/facebook/react", "topic": "hooks", "tokens": 3000}`

**Surprising result:** Both probes returned `_observed_shape: "str"` rather than a
structured dict. Context7 returns all content as formatted text strings (Markdown
documentation or library-listing text), not as JSON objects. This means there are no
fields to promote to `TypedDict` — the entire response IS the value.

## Shape decisions (Step 4)

Since both tools return `str`, the judgment is:
- `unwrap`: `[]` (no envelope to strip — the str IS the payload)
- `return_model`: `null` (a `TypedDict` cannot model a plain string)
- `fields`: `{}` (strings have no named top-level fields)
- `_observed_shape` was removed as it carries no structural information

PII scrub: `probed_args` were replaced with `<example-library-id>`, `<example-topic>`,
and `<example-query>` placeholders before committing.

## Mode table

| Tool | Mode | Reason |
|---|---|---|
| `resolve-library-id` | A (Any/str) | Returns plain Markdown text string — no dict to model |
| `query-docs` | A (Any/str) | Returns plain Markdown documentation string — no dict to model |

Both tools land in mode A (unwrap-only, returns `Any`). This is honest: the server
design deliberately returns rich text content rather than structured JSON, making
`TypedDict` modelling inapplicable.

## Path-F guard decisions

No nested fields were encountered because both responses are plain strings.
No depth-limiting decisions were required.

## Regeneration and verification

`mcp-kit codegen --shapes` regenerated `context7.py` successfully. `ast.parse`
confirmed the module is valid Python. Both functions (`query_docs`, `resolve_library_id`)
return `Any`, which correctly reflects the `str` response type.

One note: `run.py` generated by `eval-kit runner` contains `context7.query-docs()`
with a hyphenated call (not valid Python). This is a known limitation of `runner_gen`
using raw MCP tool names instead of the Python-normalized function names. The wrapper
module itself uses `query_docs` (underscore) and is correct.

## Verdict

**pass** — Both tools were probed live, shapes are honest (`str` → `Any`), and the
generated module parses cleanly.
