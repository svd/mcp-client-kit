...
def _group_by_version(results: list[dict[str, Any]], key: str) -> dict[str | None, list[str]]:
    """Map each distinct version value to the servers reporting it."""
    groups: dict[str | None, list[str]] = {}
    for result in results:
        versions = result.get("versions") or {}
        groups.setdefault(versions.get(key), []).append(result.get("server", "?"))
    return groups


def render_version_line(results: list[dict[str, Any]]) -> str:
    """One line naming the engine and skill that produced these results.

    Servers verified against different engines are flagged rather than collapsed:
    a mixed report is not comparable against any single release.
    """
    engines = _group_by_version(results, "engine")
    refs = _group_by_version(results, "skill_ref")

    if len(engines) > 1 or len(refs) > 1:
        parts = [
            f"{version or 'unknown'} ({', '.join(sorted(servers))})"
            for version, servers in sorted(
    ...
    engine_text = f"mcp-client-kit {engine}" if engine else "unknown"
    return f"Engine: {engine_text} · skill ref: {ref or 'unknown'}"
