...
The `memory` server (`@modelcontextprotocol/server-memory`) implements an in-memory
knowledge graph via stdio transport. It exposes 9 tools in total.

## Tool Inventory

**All 9 tools:**

- `create_entities` — MUTATING (create)
- `create_relations` — MUTATING (create)
- `add_observations` — MUTATING (add/write)
- `delete_entities` — MUTATING (delete)
- `delete_observations` — MUTATING (delete)
- `delete_relations` — MUTATING (delete)
- `read_graph` — non-mutating, probed
- `search_nodes` — non-mutating, probed
- `open_nodes` — non-mutating, probed

**Probed:** 3 tools. **Skipped (mutating):** 6 tools.

Skipped tools were excluded because their names match the mutating heuristic
(`create`, `delete`, `add`/`add_observations` contains "add" which implies write).
No live state changes were made to the server.

## Probe Findings

All three non-mutating tools (`read_graph`, `search_nodes`, `open_nodes`) returned the
...
The graph was empty (fresh server instance per process), so no entity or relation
objects were observed at runtime. The response shape is flat — no envelope wrapping,
no `data` or `result` outer key. The meaningful payload is the top-level dict itself,
so `unwrap` is set to `[]` (no unwrapping needed).

All three tools share an identical return shape — a `KnowledgeGraph` TypedDict with
`entities: list` and `relations: list`. Since the lists were empty, the element types
could not be inferred from live probes alone; they remain `list` (not `list[Entity]`)
to avoid modelling depth from zero observed instances.

### Notable: probe CLI exit error

The `open_nodes` probe exited with a non-zero code (`TypeError: cannot use 'list' as
a set element`) during the discriminator candidate detection phase of the CLI, after
the part file was already written successfully. The part file was valid and correct;
the error was in a post-write metadata step. This is a known mcp-kit bug triggered
by list-valued input arguments.

### No discriminator candidates

The `mcp-kit list` output reported no discriminator candidates — none of the tools
share a parameter that varies the response shape. All read tools return the same
`KnowledgeGraph` structure regardless of input.

## Mode Table

| Tool | Mode | Reason |
|---|---|---|
| `read_graph` | B | Returns dict with stable top-level keys (`entities`, `relations`); typed as `KnowledgeGraph` |
| `search_nodes` | B | Same structure as `read_graph`; query filters entities in the returned graph |
| `open_nodes` | B | Same structure; returns subgraph of requested nodes |
| `create_entities` | — | MUTATING — skipped; no live probe |
| `create_relations` | — | MUTATING — skipped; no live probe |
| `add_observations` | — | MUTATING — skipped; no live probe |
| `delete_entities` | — | MUTATING — skipped; no live probe |
| `delete_observations` | — | MUTATING — skipped; no live probe |
| `delete_relations` | — | MUTATING — skipped; no live probe |

## Path-F Guard Decisions

The `entities` and `relations` fields are lists of nested objects. Because the probe
returned empty lists, no element structure was observed. The `fields` entry records
`"entities": "list"` and `"relations": "list"` — the element TypedDicts (`Entity`,
`Relation`) were deliberately omitted to avoid asserting a depth-1 model from zero
observed instances. This follows the "don't model depth from one probe" guard.

## Generated Module Verification

`ast.parse` confirmed `memory/memory.py` is syntactically valid. The three probed
tools have return type `KnowledgeGraph` instead of `Any`. The six mutating tools
correctly remain `-> Any` (no shapes entry). The `KnowledgeGraph` TypedDict class is
emitted three times (codegen quirk for same model name across tools), but Python
handles this correctly — last definition wins, and all three are identical.
