...
def test_parse_servers_client_name_snake_alias():
    _, names, _, _ = _bridge._parse_servers({"mcpServers": {"s": {"url": "https://x/mcp", "client_name": "Snake App"}}})
...
def test_parse_servers_mixed_http_and_stdio():
    ...
    urls, _, cmds, _ = _bridge._parse_servers(
        {
            "mcpServers": {
...
def test_parse_servers_no_headers_absent():
    ...
    _, _, _, hdrs = _bridge._parse_servers({"mcpServers": {"plain": {"url": "https://x/mcp"}}})
    assert hdrs == {}
