...
The HuggingFace public MCP endpoint at `https://huggingface.co/mcp` exposes **8 tools** total. The server requires no authentication — it operates in anonymous mode with rate limiting. 7 tools were probed; 1 was skipped as mutating.

## Tool Report

**Probed (7):** `hf_whoami`, `space_search`, `hub_repo_search`, `paper_search`, `hub_repo_details`, `hf_doc_search`, `hf_doc_fetch`

**Skipped (1):**
- `gr1_z_image_turbo_generate` — name contains "generate" (image generation is a live GPU call; skipped per non-interactive gate override for mutating tools)

## Discriminator Advisory

`mcp-kit list` flagged `limit`, `offset`, and `query` as discriminator candidates (appearing across multiple tools). On inspection, these are standard pagination/search parameters that do not change the shape of the response envelope — all tools return plain text regardless of query values. No true discriminator pattern was present.

## Surprising Finding: All Tools Return Plain Text (str)

The most notable finding is that **every probed tool returns a formatted Markdown string**, not a structured JSON object. The `_observed_shape` for all 7 probed tools is `"str"`. This is unconventional for MCP tools — most servers return structured dicts. The HuggingFace server acts as a text-formatting layer over its API, returning human-readable Markdown tables and prose.

For example, `space_search` returns a Markdown table of results with columns like Space, Description, Author, ID, Category, Likes, Trending Score, Relevance. `hf_whoami` returns a plain-text instruction string about joining HuggingFace for higher rate limits.

Because all shapes are `str`, there is no envelope to unwrap, no `TypedDict` to generate, and no discriminator variants to resolve. All tools remain `-> Any` in the generated module, which is the honest representation.

## `hub_repo_details` Probe Error

A minor CLI bug was encountered probing `hub_repo_details`: the mcp-kit probe command crashed after writing the part file when the args contained a list (for `repo_ids`). The part file was written successfully, so the shape was captured. The error was `TypeError: cannot use 'list' as a set element (unhashable type: 'list')` in the probe CLI's arg-deduplication path.

## Mode Table

| Tool | Mode | Reason |
|---|---|---|
| `hf_whoami` | (any/str) | Returns plain-text instructions string |
| `space_search` | (any/str) | Returns Markdown table of space results |
| `hub_repo_search` | (any/str) | Returns Markdown table of repository results |
| `paper_search` | (any/str) | Returns Markdown table of paper results |
| `hub_repo_details` | (any/str) | Returns Markdown repo detail narrative |
| `hf_doc_search` | (any/str) | Returns Markdown documentation search results |
| `hf_doc_fetch` | (any/str) | Returns document content as plain text |
| `gr1_z_image_turbo_generate` | skipped | Mutating — image generation tool |

All probed tools fall into "unwrap-only Any" (Mode C/equivalent) since they return strings, not dicts. No `TypedDict` or `return_model` was assigned.

## Path-F Guards

No Path-F (deep nested field) guards were needed. Since all responses are `str`, there are no nested structures to worry about. The `fields` dict remains empty for all tools in the shapes.json.

## Final Module Verification

The generated `huggingface/huggingface.py` module passed `ast.parse` cleanly (8 tools, 8 async functions). All signatures return `-> Any`, which accurately reflects the server returning Markdown strings. The shapes.json contains 7 entries (the skipped `gr1_z_image_turbo_generate` has no shape entry). All `probed_args` have been PII-scrubbed: `repo_ids` replaced with `<example-repo-id>`.
