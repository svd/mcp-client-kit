...
**Environment:** `.mcp.eval.json` is already present (generated by the workflow before this agent was dispatched). Prefix every `mcp-kit` command with `MCP_KIT_SERVERS=.mcp.eval.json` in the **same shell invocation** — this forwards the manifest `env` block (e.g. `CONTEXT7_API_KEY`) to the launched process. The skill documents exact invocations for all transport types.
