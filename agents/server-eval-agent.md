...
The `--config` flag is accepted by `codegen` only — do not pass it to `list`, `probe`, or `call`.
...
Create `eval/{{SERVER_NAME}}/session-overview.draft.md`. It should be 200–500 words and cover:
...
Note: `session-overview.draft.md` is gitignored by design — do not `git add` it.
Commit only `session-overview.md`, `<server>.py`, `<server>.shapes.json`, `run.py`, `result.json`.
