...
class OrgTeams(TypedDict, total=False):
    org: str
    teams: list
...
class IssueLabels(TypedDict, total=False):
    labels: list
    totalCount: int


class IssueParent(TypedDict, total=False):
    parent: Any | None
...
class PullRequestReviewThreads(TypedDict, total=False):
    review_threads: list
    totalCount: int


class PullRequestStatus(TypedDict, total=False):
    state: str
    sha: str
    total_count: int
    statuses: list
...
class SearchCommitItem(TypedDict, total=False):
    sha: str
    html_url: str
...
class SearchPRItem(TypedDict, total=False):
    id: int
    number: int
    state: str
    locked: bool
...
async def add_issue_comment(caller: McpCaller, *, issue_number: float, owner: str, repo: str, body: str | None = None, comment_id: int | None = None, reaction: Literal['+1', '-1', 'laugh', 'confused', 'heart', 'hooray', 'rocket', 'eyes'] | None = None) -> Any:
    ...
    Args:
        issue_number: Issue or pull request number to comment on or react to.
        owner: Repository owner
        repo: Repository name
        body: Comment content. Required unless reaction is provided.
        comment_id: The numeric ID of the issue or pull request comment to react to. Use this for reactions to comments; omit it to react to the issue or pull request itself. Cannot be combined with body.
        reaction: Emoji reaction to add. Required unless body is provided.. One of: '+1', '-1', 'laugh', 'confused', 'heart', 'hooray', 'rocket', 'eyes'
    """
    args: dict[str, Any] = {"issue_number": issue_number, "owner": owner, "repo": repo}
    if body is not None:
        args["body"] = body
...
async def add_reply_to_pull_request_comment(caller: McpCaller, *, commentId: float, owner: str, repo: str, body: str | None = None, pullNumber: float | None = None, reaction: Literal['+1', '-1', 'laugh', 'confused', 'heart', 'hooray', 'rocket', 'eyes'] | None = None) -> Any:
    ...
    Args:
        commentId: The numeric ID of the pull request review comment to reply or react to. Use the number from a #discussion_r... anchor, not the GraphQL thread node ID (PRRT_...).
        owner: Repository owner
        repo: Repository name
        body: The text of the reply. Required unless reaction is provided.
        pullNumber: Pull request number. Required when body is provided.
        reaction: Emoji reaction to add. Required unless body is provided.. One of: '+1', '-1', 'laugh', 'confused', 'heart', 'hooray', 'rocket', 'eyes'
    """
    args: dict[str, Any] = {"commentId": commentId, "owner": owner, "repo": repo}
    if body is not None:
        args["body"] = body
...
create_pull_request.__schema__ = {'type': 'object', 'properties': {'base': {'type': 'string', 'description': 'Branch to merge into'}, 'body': {'type': 'string', 'description': 'PR description'}, 'draft': {'type': 'boolean', 'description': 'Create as draft PR'}, 'head': {'type': 'string', 'description': 'Branch containing changes'}, 'maintainer_can_modify': {'type': 'boolean', 'description': 'Allow maintainer edits'}, 'owner': {'type': 'string', 'description': 'Repository owner', 'x-mcp-header': 'owner'}, 'repo': {'type': 'string', 'description': 'Repository name', 'x-mcp-header': 'repo'}, 'reviewers': {'type': 'array', 'items': {'type': 'string'}, 'description': 'GitHub usernames or ORG/team-slug team reviewers to request reviews from'}, 'title': {'type': 'string', 'description': 'PR title'}}, 'required': ['owner', 'repo', 'title', 'head', 'base']}
...
delete_file.__schema__ = {'type': 'object', 'properties': {'branch': {'type': 'string', 'description': 'Branch to delete the file from'}, 'message': {'type': 'string', 'description': 'Commit message'}, 'owner': {'type': 'string', 'description': 'Repository owner (username or organization)', 'x-mcp-header': 'owner'}, 'path': {'type': 'string', 'description': 'Path to the file to delete'}, 'repo': {'type': 'string', 'description': 'Repository name', 'x-mcp-header': 'repo'}}, 'required': ['owner', 'repo', 'path', 'message', 'branch']}
...
fork_repository.__schema__ = {'type': 'object', 'properties': {'organization': {'type': 'string', 'description': 'Organization to fork to'}, 'owner': {'type': 'string', 'description': 'Repository owner', 'x-mcp-header': 'owner'}, 'repo': {'type': 'string', 'description': 'Repository name', 'x-mcp-header': 'repo'}}, 'required': ['owner', 'repo']}


async def get_commit(caller: McpCaller, *, owner: str, repo: str, sha: str, detail: Literal['none', 'stats', 'full_patch'] | None = None, page: float | None = None, perPage: float | None = None) -> Commit:
    ...
    return cast("Commit", await caller.call(SERVER, "get_commit", args))
...
get_label.__schema__ = {'type': 'object', 'properties': {'name': {'type': 'string', 'description': 'Label name.'}, 'owner': {'type': 'string', 'description': 'Repository owner (username or organization name)', 'x-mcp-header': 'owner'}, 'repo': {'type': 'string', 'description': 'Repository name', 'x-mcp-header': 'repo'}}, 'required': ['owner', 'repo', 'name']}
...
get_latest_release.__schema__ = {'type': 'object', 'properties': {'owner': {'type': 'string', 'description': 'Repository owner', 'x-mcp-header': 'owner'}, 'repo': {'type': 'string', 'description': 'Repository name', 'x-mcp-header': 'repo'}}, 'required': ['owner', 'repo']}


async def get_me(caller: McpCaller) -> AuthenticatedUser:
    """Get details of the authenticated GitHub user. Use this when a request is about the user's own profile for GitHub. Or when information is missing to build other tool calls."""
    return cast("AuthenticatedUser", await caller.call(SERVER, "get_me", {}))
...
async def get_tag(caller: McpCaller, *, owner: str, repo: str, tag: str) -> Tag:
    ...
    """
    args: dict[str, Any] = {"owner": owner, "repo": repo, "tag": tag}
    return cast("Tag", await caller.call(SERVER, "get_tag", args))
...
async def get_teams(caller: McpCaller, *, user: str | None = None) -> list[OrgTeams]:
    ...
    if user is not None:
        args["user"] = user
    return cast("list[OrgTeams]", await caller.call(SERVER, "get_teams", args))
...
@overload
async def issue_read(caller: McpCaller, *, issue_number: float, method: Literal['get'], owner: str, repo: str, page: float | None = None, perPage: float | None = None) -> Issue: ...

@overload
async def issue_read(caller: McpCaller, *, issue_number: float, method: Literal['get_labels'], owner: str, repo: str, page: float | None = None, perPage: float | None = None) -> IssueLabels: ...

@overload
async def issue_read(caller: McpCaller, *, issue_number: float, method: Literal['get_parent'], owner: str, repo: str, page: float | None = None, perPage: float | None = None) -> IssueParent: ...

async def issue_read(caller: McpCaller, *, issue_number: float, method: str, owner: str, repo: str, page: float | None = None, perPage: float | None = None) -> Issue | IssueLabels | IssueParent:
    ...
    5. get_labels - Get labels assigned to the issue.. One of: 'get', 'get_comments', 'get_sub_issues', 'get_parent', 'get_labels'
    ...
    if perPage is not None:
        args["perPage"] = perPage
    return cast("Issue | IssueLabels | IssueParent", await caller.call(SERVER, "issue_read", args))
...
async def issue_write(caller: McpCaller, *, method: Literal['create', 'update'], owner: str, repo: str, assignees: list[str] | None = None, body: str | None = None, duplicate_of: float | None = None, issue_fields: list[dict] | None = None, issue_number: float | None = None, labels: list[str] | None = None, milestone: float | None = None, parent_issue_number: float | None = None, parent_owner: str | None = None, parent_repo: str | None = None, state: Literal['open', 'closed'] | None = None, state_reason: Literal['completed', 'not_planned', 'duplicate'] | None = None, title: str | None = None, type: Any | None = None) -> Any:
    ...
    - 'update' - updates an existing issue.. One of: 'create', 'update'
        ...
        parent_issue_number: Issue number of the parent issue. Only used when method is 'create' and cannot be combined with issue_fields. The new issue is created and attached to this parent in the same operation.
        parent_owner: Repository owner of the parent issue. Must be provided with parent_repo. Omit both to use owner and repo. Only used when method is 'create' and parent_issue_number is provided.
        parent_repo: Repository name of the parent issue. Must be provided with parent_owner. Omit both to use owner and repo. Only used when method is 'create' and parent_issue_number is provided.
    ...
    if parent_issue_number is not None:
        args["parent_issue_number"] = parent_issue_number
    if parent_owner is not None:
        args["parent_owner"] = parent_owner
    if parent_repo is not None:
        args["parent_repo"] = parent_repo
...
list_issue_fields.__schema__ = {'type': 'object', 'properties': {'owner': {'type': 'string', 'description': 'The account owner of the repository or organization. The name is not case sensitive.', 'x-mcp-header': 'owner'}, 'repo': {'type': 'string', 'description': 'The name of the repository. When provided, returns fields for this specific repository (inherited from its organization). When omitted, returns org-level fields directly.', 'x-mcp-header': 'repo'}}, 'required': ['owner']}


async def list_issue_types(caller: McpCaller, *, owner: str, repo: str | None = None) -> list[IssueType]:
    ...
    if repo is not None:
        args["repo"] = repo
    return cast("list[IssueType]", await caller.call(SERVER, "list_issue_types", args))
...
list_repository_collaborators.__schema__ = {'type': 'object', 'properties': {'affiliation': {'type': 'string', 'description': "Filter by affiliation. Can be one of: 'outside' (outside collaborators), 'direct' (all with permissions regardless of org membership), 'all' (all collaborators). Default: 'all'", 'enum': ['outside', 'direct', 'all']}, 'owner': {'type': 'string', 'description': 'Repository owner', 'x-mcp-header': 'owner'}, 'page': {'type': 'number', 'description': 'Page number for pagination (default 1, min 1)', 'minimum': 1}, 'perPage': {'type': 'number', 'description': 'Results per page for pagination (default 30, min 1, max 100)', 'minimum': 1, 'maximum': 100}, 'repo': {'type': 'string', 'description': 'Repository name', 'x-mcp-header': 'repo'}}, 'required': ['owner', 'repo']}
...
merge_pull_request.__schema__ = {'type': 'object', 'properties': {'commit_message': {'type': 'string', 'description': 'Extra detail for merge commit'}, 'commit_title': {'type': 'string', 'description': 'Title for merge commit'}, 'merge_method': {'type': 'string', 'description': 'Merge method', 'enum': ['merge', 'squash', 'rebase']}, 'owner': {'type': 'string', 'description': 'Repository owner', 'x-mcp-header': 'owner'}, 'pullNumber': {'type': 'number', 'description': 'Pull request number'}, 'repo': {'type': 'string', 'description': 'Repository name', 'x-mcp-header': 'repo'}}, 'required': ['owner', 'repo', 'pullNumber']}


@overload
async def pull_request_read(caller: McpCaller, *, method: Literal['get'], owner: str, pullNumber: float, repo: str, after: str | None = None, page: float | None = None, perPage: float | None = None) -> PullRequest: ...

@overload
async def pull_request_read(caller: McpCaller, *, method: Literal['get_review_comments'], owner: str, pullNumber: float, repo: str, after: str | None = None, page: float | None = None, perPage: float | None = None) -> PullRequestReviewThreads: ...

@overload
async def pull_request_read(caller: McpCaller, *, method: Literal['get_status'], owner: str, pullNumber: float, repo: str, after: str | None = None, page: float | None = None, perPage: float | None = None) -> PullRequestStatus: ...

async def pull_request_read(caller: McpCaller, *, method: str, owner: str, pullNumber: float, repo: str, after: str | None = None, page: float | None = None, perPage: float | None = None) -> PullRequest | PullRequestReviewThreads | PullRequestStatus:
    ...
    if perPage is not None:
        args["perPage"] = perPage
    return cast("PullRequest | PullRequestReviewThreads | PullRequestStatus", await caller.call(SERVER, "pull_request_read", args))

pull_request_read.__schema__ = {'type': 'object', 'properties': {'after': {'type': 'string', 'description': "Cursor for pagination, used only by the get_review_comments method. Pass the endCursor from the previous page's PageInfo to fetch the next page."}, 'method': {'type': 'string', 'description': "Action to specify what pull request data needs to be retrieved from GitHub. \nPossible options: \n 1. get - Get details of a specific pull request.\n 2. get_diff - Get the diff of a pull request.\n 3. get_status - Get combined commit status of a head commit in a pull request.\n 4. get_files - Get the list of files changed in a pull request. Use with pagination parameters to control the number of results returned.\n 5. get_commits - Get the list of commits on a pull request. Use with pagination parameters to control the number of results returned.\n 6. get_review_comments - Get review threads on a pull request. Each thread contains logically grouped review comments made on the same code location during pull request reviews. Returns threads with metadata (isResolved, isOutdated, isCollapsed) and their associated comments. Use cursor-based pagination (perPage, after) to control results.\n 7. get_reviews - Get the reviews on a pull request. When asked for review comments, use get_review_comments method. Use with pagination parameters to control the number of results returned.\n 8. get_comments - Get comments on a pull request. Use this if user doesn't specifically want review comments. Use with pagination parameters to control the number of results returned.\n 9. get_check_runs - Get check runs for the head commit of a pull request. Check runs are the individual CI/CD jobs and checks that run on the PR.\n", 'enum': ['get', 'get_diff', 'get_status', 'get_files', 'get_commits', 'get_review_comments', 'get_reviews', 'get_comments', 'get_check_runs']}, 'owner': {'type': 'string', 'description': 'Repository owner', 'x-mcp-header': 'owner'}, 'page': {'type': 'number', 'description': 'Page number for pagination (min 1)', 'minimum': 1}, 'perPage': {'type': 'number', 'description': 'Results per page for pagination (min 1, max 100)', 'minimum': 1, 'maximum': 100}, 'pullNumber': {'type': 'number', 'description': 'Pull request number'}, 'repo': {'type': 'string', 'description': 'Repository name', 'x-mcp-header': 'repo'}}, 'required': ['method', 'owner', 'repo', 'pullNumber']}
...
pull_request_review_write.__schema__ = {'type': 'object', 'properties': {'body': {'type': 'string', 'description': 'Review comment text'}, 'commitID': {'type': 'string', 'description': 'SHA of commit to review'}, 'event': {'type': 'string', 'description': 'Review action to perform.', 'enum': ['APPROVE', 'REQUEST_CHANGES', 'COMMENT']}, 'method': {'type': 'string', 'description': 'The write operation to perform on pull request review.', 'enum': ['create', 'submit_pending', 'delete_pending', 'resolve_thread', 'unresolve_thread']}, 'owner': {'type': 'string', 'description': 'Repository owner', 'x-mcp-header': 'owner'}, 'pullNumber': {'type': 'number', 'description': 'Pull request number'}, 'repo': {'type': 'string', 'description': 'Repository name', 'x-mcp-header': 'repo'}, 'threadId': {'type': 'string', 'description': 'The node ID of the review thread (e.g., PRRT_kwDOxxx). Required for resolve_thread and unresolve_thread methods. Get thread IDs from pull_request_read with method get_review_comments.'}}, 'required': ['method', 'owner', 'repo', 'pullNumber']}
...
push_files.__schema__ = {'type': 'object', 'properties': {'branch': {'type': 'string', 'description': 'Branch to push to'}, 'files': {'type': 'array', 'items': {'type': 'object', 'properties': {'content': {'type': 'string', 'description': 'file content'}, 'path': {'type': 'string', 'description': 'path to the file'}}, 'required': ['path', 'content'], 'additionalProperties': False}, 'description': 'Array of file objects to push, each object with path (string) and content (string)'}, 'message': {'type': 'string', 'description': 'Commit message'}, 'owner': {'type': 'string', 'description': 'Repository owner', 'x-mcp-header': 'owner'}, 'repo': {'type': 'string', 'description': 'Repository name', 'x-mcp-header': 'repo'}}, 'required': ['owner', 'repo', 'branch', 'files', 'message']}
...
async def search_code(caller: McpCaller, *, query: str, fields: list[Literal['name', 'path', 'sha', 'repository', 'text_matches']] | None = None, order: Literal['asc', 'desc'] | None = None, page: float | None = None, perPage: float | None = None, sort: str | None = None) -> list[SearchCodeItem]:
    ...
    if sort is not None:
        args["sort"] = sort
    result = await caller.call(SERVER, "search_code", args)
    return cast("list[SearchCodeItem]", _dig_list(result, ('items', )))
...
async def search_commits(caller: McpCaller, *, query: str, order: Literal['asc', 'desc'] | None = None, page: float | None = None, perPage: float | None = None, sort: Literal['author-date', 'committer-date'] | None = None) -> list[SearchCommitItem]:
    ...
    if sort is not None:
        args["sort"] = sort
    result = await caller.call(SERVER, "search_commits", args)
    return cast("list[SearchCommitItem]", _dig_list(result, ('items', )))
...
async def search_issues(caller: McpCaller, *, query: str, fields: list[Literal['number', 'title', 'body', 'state', 'state_reason', 'draft', 'locked', 'html_url', 'user', 'author_association', 'labels', 'assignee', 'assignees', 'milestone', 'comments', 'reactions', 'created_at', 'updated_at', 'closed_at', 'closed_by', 'type', 'repository_url', 'pull_request', 'field_values']] | None = None, order: Literal['asc', 'desc'] | None = None, owner: str | None = None, page: float | None = None, perPage: float | None = None, repo: str | None = None, sort: Literal['comments', 'reactions', 'reactions-+1', 'reactions--1', 'reactions-smile', 'reactions-thinking_face', 'reactions-heart', 'reactions-tada', 'interactions', 'created', 'updated'] | None = None) -> list[SearchIssueItem]:
    ...
    if sort is not None:
        args["sort"] = sort
    result = await caller.call(SERVER, "search_issues", args)
    return cast("list[SearchIssueItem]", _dig_list(result, ('items', )))
...
async def search_pull_requests(caller: McpCaller, *, query: str, fields: list[Literal['number', 'title', 'body', 'state', 'state_reason', 'draft', 'locked', 'html_url', 'user', 'author_association', 'labels', 'assignee', 'assignees', 'milestone', 'comments', 'reactions', 'created_at', 'updated_at', 'closed_at', 'closed_by', 'pull_request', 'repository_url']] | None = None, order: Literal['asc', 'desc'] | None = None, owner: str | None = None, page: float | None = None, perPage: float | None = None, repo: str | None = None, sort: Literal['comments', 'reactions', 'reactions-+1', 'reactions--1', 'reactions-smile', 'reactions-thinking_face', 'reactions-heart', 'reactions-tada', 'interactions', 'created', 'updated'] | None = None) -> list[SearchPRItem]:
    ...
    if sort is not None:
        args["sort"] = sort
    result = await caller.call(SERVER, "search_pull_requests", args)
    return cast("list[SearchPRItem]", _dig_list(result, ('items', )))
...
async def search_repositories(caller: McpCaller, *, query: str, minimal_output: bool | None = None, order: Literal['asc', 'desc'] | None = None, page: float | None = None, perPage: float | None = None, sort: Literal['stars', 'forks', 'help-wanted-issues', 'updated'] | None = None) -> list[SearchRepositoryItem]:
    ...
    if sort is not None:
        args["sort"] = sort
    result = await caller.call(SERVER, "search_repositories", args)
    return cast("list[SearchRepositoryItem]", _dig_list(result, ('items', )))
...
async def search_users(caller: McpCaller, *, query: str, order: Literal['asc', 'desc'] | None = None, page: float | None = None, perPage: float | None = None, sort: Literal['followers', 'repositories', 'joined'] | None = None) -> list[SearchUserItem]:
    ...
    if sort is not None:
        args["sort"] = sort
    result = await caller.call(SERVER, "search_users", args)
    return cast("list[SearchUserItem]", _dig_list(result, ('items', )))
...
update_pull_request.__schema__ = {'type': 'object', 'properties': {'base': {'type': 'string', 'description': 'New base branch name'}, 'body': {'type': 'string', 'description': 'New description'}, 'draft': {'type': 'boolean', 'description': 'Mark pull request as draft (true) or ready for review (false)'}, 'maintainer_can_modify': {'type': 'boolean', 'description': 'Allow maintainer edits'}, 'owner': {'type': 'string', 'description': 'Repository owner', 'x-mcp-header': 'owner'}, 'pullNumber': {'type': 'number', 'description': 'Pull request number to update'}, 'repo': {'type': 'string', 'description': 'Repository name', 'x-mcp-header': 'repo'}, 'reviewers': {'type': 'array', 'items': {'type': 'string'}, 'description': 'GitHub usernames or ORG/team-slug team reviewers to request reviews from'}, 'state': {'type': 'string', 'description': 'New state', 'enum': ['open', 'closed']}, 'title': {'type': 'string', 'description': 'New title'}}, 'required': ['owner', 'repo', 'pullNumber']}
...
update_pull_request_branch.__schema__ = {'type': 'object', 'properties': {'expectedHeadSha': {'type': 'string', 'description': "The expected SHA of the pull request's HEAD ref"}, 'owner': {'type': 'string', 'description': 'Repository owner', 'x-mcp-header': 'owner'}, 'pullNumber': {'type': 'number', 'description': 'Pull request number'}, 'repo': {'type': 'string', 'description': 'Repository name', 'x-mcp-header': 'repo'}}, 'required': ['owner', 'repo', 'pullNumber']}
