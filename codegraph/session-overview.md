...
# Session Overview: codegraph MCP Wrappers

## Server

**codegraph** — local code-intelligence MCP server backed by a SQLite knowledge graph. Transport: stdio via `codegraph serve --mcp` (binary at `/opt/homebrew/bin/codegraph`). No auth required.

## Tool Inventory

The server exposes **5 tools** via `tools/list`. All are non-mutating read operations — none of their names contain any of the mutating keywords (create, update, delete, remove, send, set, write, post, patch, put, cancel, approve, submit, assign). Consequently **all 5 tools were probed** and **0 tools were skipped**.

## Probing

Each tool was probed once via `mcp-kit probe` with minimal safe arguments. The probe used realistic but non-sensitive symbol names (`McpCaller`, `McpBridgeCaller`) and the eval project path as `projectPath`. Sample identifiers were scrubbed before committing (real paths and symbol names replaced with `<example-*>` placeholders).

A notable discovery during setup: the servers.toml config lists `codegraph-mcp` as the launch command, but the installed binary is actually `codegraph serve --mcp` (with the binary at `/opt/homebrew/bin/codegraph`). The eval-kit runner script (`run.py`) was generated using the `codegraph-mcp` alias from servers.toml — users may need to adjust to `codegraph serve --mcp` if that alias is not on PATH.

## Response Shape

All 5 tools returned `_observed_shape: "str"` — the server returns **plain text** (markdown-formatted reports, symbol listings, code excerpts) rather than structured JSON. This is intentional and consistent with the server's design as a human-readable code intelligence assistant.

There are no JSON envelopes to unwrap, no list containers, and no scalar fields to extract into a `TypedDict`. All tools appropriately remain `-> Any` in the generated module (which accurately reflects `str` at runtime).

## Discriminator Assessment

`mcp-kit list` flagged two discriminator candidates:
- `projectPath` — spans all 5 tools
- `query` — spans `codegraph_explore` and `codegraph_search`

In both cases, the "discriminator" is a query/selector parameter, not a shape discriminator. All tools return the same type (`str`) regardless of the value of `projectPath` or `query`. The structure of the response does not vary by parameter value — only the content does. **No discriminator variants were defined** (generic base / unwrap-only `Any` applies per gate override rule #2).

## Mode Table

| Tool | Mode | Reason |
|---|---|---|
| `codegraph_search` | A | Returns plain `str` — no dict structure to model |
| `codegraph_context` | A | Returns plain `str` — no dict structure to model |
| `codegraph_node` | A | Returns plain `str` — no dict structure to model |
| `codegraph_explore` | A | Returns plain `str` — no dict structure to model |
| `codegraph_trace` | A | Returns plain `str` — no dict structure to model |

## Path-F Guard

No Path-F (nested field omission) decisions were needed — all responses are flat strings with no nested structure.

## Generation Outcome

The regenerated `codegraph/codegraph.py` parsed cleanly via `ast.parse`. All 5 function signatures are correctly typed from `inputSchema`, return `Any`, and forward arguments through `McpCaller.call`. The shapes sidecar `codegraph/codegraph.shapes.json` contains PII-scrubbed entries for all 5 probed tools with honest `source: "live"` attestation.
