...
# Session Overview: github MCP Server Wrapper Generation

## Server Summary

The GitHub Copilot MCP server at `https://api.githubcopilot.com/mcp/` exposes **44 tools** total. Of these, **27 were probed** (non-mutating) and **17 were skipped** (mutating).

## Tool Selection

### Probed (non-mutating, 27 tools)
`get_commit`, `get_file_contents`, `get_label`, `get_latest_release`, `get_release_by_tag`, `get_tag`, `get_me`, `get_team_members`, `get_teams`, `issue_read`, `list_branches`, `list_commits`, `list_issue_fields`, `list_issue_types`, `list_issues`, `list_pull_requests`, `list_releases`, `list_repository_collaborators`, `list_tags`, `pull_request_read`, `run_secret_scanning`, `search_code`, `search_commits`, `search_issues`, `search_pull_requests`, `search_repositories`, `search_users`

### Skipped (mutating, 17 tools)
`add_comment_to_pending_review`, `add_issue_comment`, `add_reply_to_pull_request_comment`, `create_branch`, `create_or_update_file`, `create_pull_request`, `create_repository`, `delete_file`, `fork_repository`, `issue_write`, `merge_pull_request`, `pull_request_review_write`, `push_files`, `request_copilot_review`, `sub_issue_write`, `update_pull_request`, `update_pull_request_branch` — all contain create/update/delete/merge/write/push/fork keywords.

## Interesting Observations

- **Many tools return formatted strings (`_observed_shape: "str"`)**: `get_commit`, `get_file_contents`, `list_branches`, `list_commits`, `list_issues`, `list_pull_requests`, `list_repository_collaborators`, `list_issue_fields`, `list_issue_types`, `run_secret_scanning`, `search_commits`, `pull_request_read`, `get_team_members`. The GitHub MCP server appears to render many responses as markdown text rather than structured JSON — this is atypical and means TypedDicts can't be emitted for these tools (they stay `-> Any`).
- **`list_releases` failed on a private repo** (404) — re-probed against `microsoft/vscode` (public), which worked and revealed a structured list response with 30 releases per page.
- **`run_secret_scanning` probe succeeded** but the probe CLI had a display-level bug (TypeError on `list` as set element in probed_args summary) that didn't affect the written part file.
- **`get_teams` returns a list of `{org, teams}` objects** but teams array was empty in this probe (`"<empty>"`), so no team TypedDict was emitted — left as `Any`.
- **Discriminators flagged by `mcp-kit list`** were shared parameter names (owner, repo, method, page, etc.) that do NOT indicate polymorphic response shapes — they are common pagination/routing params. No true discriminated union shapes were found. Resolved to generic base models where shapes were stable.
- **Search tools** (`search_issues`, `search_pull_requests`, `search_repositories`, `search_users`, `search_code`) share the same outer `{total_count, incomplete_results, items}` envelope, unified under `SearchResult`.

## Mode Table

| Tool | Mode | Reason |
|---|---|---|
| `get_me` | B | Returns dict with stable scalar fields (login, id, profile_url, avatar_url) |
| `get_label` | B | Returns dict with color, description, id, name |
| `get_latest_release` | B | Returns dict with full release fields |
| `get_release_by_tag` | B | Same release shape as get_latest_release |
| `get_tag` | B | Returns dict with ref, url, node_id |
| `issue_read` | B | Returns dict with issue scalar fields (number, title, state, etc.) |
| `list_releases` | B+list | Returns list of release dicts |
| `list_tags` | B+list | Returns list of {name, sha} tag refs |
| `search_code` | B | Returns {total_count, incomplete_results, items} envelope |
| `search_issues` | B | Same search envelope |
| `search_pull_requests` | B | Same search envelope |
| `search_repositories` | B | Same search envelope |
| `search_users` | B | Same search envelope |
| `get_commit` | A | Returns str (markdown-formatted commit text) |
| `get_file_contents` | A | Returns str (file contents as text) |
| `list_branches` | A | Returns str (formatted list) |
| `list_commits` | A | Returns str (formatted list) |
| `list_issues` | A | Returns str (formatted list) |
| `list_pull_requests` | A | Returns str (formatted list) |
| `list_repository_collaborators` | A | Returns str (formatted list) |
| `list_issue_fields` | A | Returns str |
| `list_issue_types` | A | Returns str |
| `pull_request_read` | A | Returns str (formatted PR details) |
| `search_commits` | A | Returns str |
| `run_secret_scanning` | A | Returns str (scan result text) |
| `get_team_members` | A | Returns str (member list text) |
| `get_teams` | A | Returns list with empty teams arrays — no stable shape |

## Path-F Guard Decisions

- Deep nested fields (e.g. `author` objects with 20+ URL template fields in release/issue responses, `reactions` dicts, `milestone` objects in issue search) were omitted from `fields` — only top-level stable scalars promoted.
- `search_issues` items contain deeply nested user/assignee/milestone/label structures — kept `items` field out of the TypedDict, relying on `total_count` and `incomplete_results` as the shaped scalars.

## Generated Module

The final `github/github.py` (30,127 bytes) parsed cleanly via `ast.parse`. Shaped tools have correct return signatures: `-> Label`, `-> Release`, `-> GitHubUser`, `-> GitTag`, `-> Issue`, `-> SearchResult`, `-> list[Release]`, `-> list[TagRef]`. Duplicate class definitions for `Release` and `SearchResult` (shared across multiple tools) are expected codegen behavior — last definition wins in Python, fields are identical.
